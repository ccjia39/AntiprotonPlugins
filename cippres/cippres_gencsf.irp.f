...
subroutine generate_csfs(finput)
 ...
 integer :: n_ciruns, n_csfmax, n_detmax
...
! the python script will write the info and list of CSFs, for as many CIruns as given in finput, into header$irun.txt and list$irun.txt ($irun=1,2,3,....)
  ...
  call system('python $QP_ROOT/plugins/qp_plugins_kao/cippres/libs/generate_csfs.py '//trim(finput))
! save the info and list of CSFs into EZFIO
 ...
 allocate(csf_basis_tmp(N_int,2, n_detmax, n_csfmax ,n_ciruns))
 allocate(occ_a(mo_num,n_detmax), occ_b(mo_num,n_detmax)) 
 allocate(nsta(n_ciruns),prttol(n_ciruns),ncsf(n_ciruns))
 allocate(ndet(n_csfmax,n_ciruns),coefdet(n_detmax,n_csfmax,n_ciruns))
